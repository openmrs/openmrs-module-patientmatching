...
public class MatchResultReviewPanel extends JPanel implements ActionListener, ChangeListener, KeyListener {
	...
	public void setMatchResult(MatchResult mr) {
		...
		for (int i = 0; i < values.getColumnCount(); i++) {
			...
			for (int r = 0; r < values.getRowCount(); r++) {
				...
				comp = renderer.getTableCellRendererComponent(values, values.getValueAt(r, vColIndex), false, false, r,
				    vColIndex);
		...
		certainty_val.setText(
		    (mr.getMatch_status() == MatchResult.UNKNOWN) ? "" : Integer.toString(getUICertaintyLevel(mr.getCertainty())));
	...
	private void setMatchResultCertainty(MatchResult mr, int certainty) {
		if (mr == null) {
			...
			MatchResultReviewPagerPanel.printReviewLog(rowValue + "|" + mr.getRecord1().getUID() + "|"
			        + mr.getRecord2().getUID() + "|" + certainty + "|" + now.getTime() + "|" + now);
