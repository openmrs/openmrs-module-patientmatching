...
/**
 * As of July 18th, class no longer being used; instad moving to each analyzer implementing a
 * modifier interface Class is used by ScorePair to modify the score used when comparing two
 * Records. It contains a list of Record objects that store regular expressions as demographic
 * values. If a Record matches the pattern Record, then the scaling is returned. For example, if you
 * need to get the modification for the Record - first_name - foo last_name - bar yb - 1984 and
 * there is a pattern Record of: first_name - foo then it will match and the scaling returned. It is
 * a Record that has the effect of "all Record objects with a first_name value of 'foo' will be
 * scaled by <scale>" If the pattern Record were: first_name - * then it will match every first name
 * and all Records will be scaled.
 * 
 * @author jegg
 */
...
public class ScoreModifier {
	...
	/**
	 * Method adds a scaling for every Record for the given demographic, regardless of value in that
	 * Record.
	 * 
	...
	/**
	 * Method adds a scaling for every Record for the given demographic only when the value of the
	 * demographic matches the regular expression.
	 * 
	 * @param demographic the column/demographic that is to be modified
