...
/**
 * Class implements a special case of reading from a database where the records in the database will
 * only form pairs against one Record. Instead of sorting the records by the blocking columns, an
 * SQL query can retrieve only the records that match the blocking columns of the record we're
 * matching against.
 * 
 * @author jegg
 */
...
public class SubsetDataBaseReader extends OrderedDataBaseReader implements SubsetDataSourceReader {
	...
	/**
	 * Overridden method adds a "WHERE" clause to the SQL to only get rows that match the blocking
	 * column values of the to_match Record.
	 */
