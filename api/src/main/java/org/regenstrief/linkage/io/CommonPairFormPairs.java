...
/**
 * Class is a FormPairs object that takes a list of other FormPairs in its constructor. It passes
 * through pairs from the multiple FormPairs, but each pair is only passed through once. The test
 * for equivalency currently depends on which side of the pair an ID appears. So pair 1,2 and 2,1
 * will both be passed through even if the pair is actually the same.
 */
...
public class CommonPairFormPairs extends FormPairs {
	...
	/**
	 * Method returns whether the two records wiould be paired in any of the MatchingConfig objects in
	 * mcs
	 * 
	 * @param r1 the first Record
	 * @param r2 the second Record
	 * @return true if the Records have equal blocking column values for any MatchingConfig objects in
	 *         mcs
	 */
