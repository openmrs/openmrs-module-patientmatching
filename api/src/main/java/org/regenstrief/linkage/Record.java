...
/**
 * Record stores demographics associated with one entity. The values are stored in a Hashtable
 * indexed on the type of information. For example, an examlpe of a demographic for first name would
 * be stored in the Hashtable with a key of "FN" and a value of "John." Unique ID should be a unique
 * integer, such that all Records from a given datasource receives a different integer ID. Also,
 * different datasources should have a unique uid_context so that same ID records from different
...
public class Record {
	...
	/**
	 * Method returns true if any of the demographics in this Record has a null or empty string as its
	 * value
	 * 
	 * @return true if at least one demographic has no value, false if all demographics have non-empty
	 *         string, non-null values
	 */
	...
	/**
	 * Adds an entry to the demographics table with value of the value parameter. Previous values, if
	 * present, are over written.
	 * 
	...
	/**
	 * Determines whether or not a given demographic field is considered null. We assume that a field is
	 * null if and only if it is an empty string.
	 * 
	 * @param demographic the name of the demographic required
