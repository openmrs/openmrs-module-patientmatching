...
public class SavedResultDBConnection {
	...
	public static final String CREATE_MATCH_RESULT_TABLE = "create table matchresult(" +
			...
			"note varchar(255)," +
			"foreign key(uid1) REFERENCES record(uid)," +
	...
	public static final String CREATE_DEMOGRAPHIC_TABLE = "create table demographic(" +
			...
			"value varchar(40)," +
			"foreign key(uid) REFERENCES record(uid)" +
			")";
	...
	public static void validateMatchResultTables(final Connection db) {
		...
		try {
			...
			validate0(st, "select count(1) from matchresult m where not exists(select 1 from demographic d where d.uid=m.uid1)");
			validate0(st, "select count(1) from matchresult m where not exists(select 1 from demographic d where d.uid=m.uid2)");
			st.close();
		} catch (final SQLException e) {
			throw new RuntimeException(e);
		}
