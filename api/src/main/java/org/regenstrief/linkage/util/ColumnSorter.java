...
public class ColumnSorter {
	...
	*/
	static String[] possibleWindowsLocations = { "cygwin\\bin\\sort.exe", "cygwin64\\bin\\sort.exe",
	        "UnxUtils\\usr\\local\\wbin\\sort.exe", "GnuWin32\\bin\\sort.exe", "win32\\bin\\sort.exe", "win32\\sort.exe",
	        "C:\\cygwin\\bin\\sort.exe", "C:\\cygwin64\\bin\\sort.exe", "C:\\UnxUtils\\usr\\local\\wbin\\sort.exe",
	...
	private static boolean sortIntern(char separator, Vector<ColumnSortOption> options, File input_file, File output_file)
	        throws Exception {
		...
		if (sort_command != null) { // && ((can_sort_parallel || !enough_mem_for_internal_sort)) {
			...
			if (os_name.indexOf("Windows") != -1) {
				...
				if ((tmp_file != null) && tmp_file.exists() && tmp_file.isDirectory()) {
					...
					System.err.println(
					    "Could not find or create temporary directory (" + tmp_dirname + ") for the sort program.");
					num_command_args -= 1;
				}
			}
