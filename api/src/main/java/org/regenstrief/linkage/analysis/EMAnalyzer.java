...
/**
 * Class was originally extending Analyzer, but EM analyzes pairs of records, not a record data
 * source, so it no longer is a subclass. It also modifies the MatchingConfig object given in the
 * analyzeRecordPairs method with new agreement and non-agreement values for included columns as a
 * side effect
 */
...
public class EMAnalyzer extends RecordPairAnalyzer implements LoggingObject { //extends Analyzer {
	...
	/**
	 * Constructor needs a name to use when created the temporary .vct file
	 */
	...
	public void finishAnalysis() {
		...
		for (int i = 0; i < demographics.length; i++) {
			...
			logBuffer.append("Initializing demographic: ").append(demographics[i]).append(" u = ")
			        .append(uest.get(demographics[i])).append(" and m = ").append(mest.get(demographics[i]));
		...
		for (int i = 0; i < iterations; i++) {
			...
			for (int j = 0; j < demographics.length; j++) {
				...
				if (!mc.getMatchingConfigRowByName(demographic).isTrinomialEM()) {
					...
					log.info(demographic + " p est: " + psum.get(demographic) + " * " + Cmsum + " / " + mdenom + " = "
					        + pest_val);
					log.info(demographic + " q est: " + qsum.get(demographic) + " * " + Cmsum + " / " + mdenom + " = "
					        + qest_val);
					
					log.info(demographic + " s est: " + ssum.get(demographic) + " * " + Cusum + " / " + udenom + " = "
					        + sest_val);
					log.info(demographic + " t est: " + tsum.get(demographic) + " * " + Cusum + " / " + udenom + " = "
					        + test_val);
			...
			for (int j = 0; j < demographics.length; j++) {
				String demographic = demographics[j];
				if (mc.getMatchingConfigRowByName(demographic).isTrinomialEM()) {
					log.info(demographic + ":   pest: " + pest.get(demographic) + "   qest: " + qest.get(demographic)
					        + "   sest: " + sest.get(demographic) + "   test: " + test.get(demographic));
				} else {
		...
		while (it.hasNext()) {
			...
			mrs.add(new MatchResult(vt.getScore(mv), vt.getInclusiveScore(mv), vt.getMatchVectorFalseProbability(mv),
			        vt.getMatchVectorTrueProbability(mv), vt.getSensitivity(mv), vt.getSpecificity(mv), mv, null, null, null,
			        mc));
		}
	...
	protected void modifyMU(MatchVector mv, double p, int count) {
		...
		while (it.hasNext()) {
			...
			if (avg_em) {
				...
				termM = termM
				        * Math.pow((Math.pow(mest.get(demographic), comp) * Math.pow(1 - mest.get(demographic), 1 - comp)),
				            bin)
				        * Math.pow((Math.pow(pest.get(demographic), A) * Math.pow(qest.get(demographic), B)
				                * Math.pow((1 - pest.get(demographic) - qest.get(demographic)), C)),
				            1 - bin);
				
				termU = termU
				        * Math.pow((Math.pow(uest.get(demographic), comp) * Math.pow(1 - uest.get(demographic), 1 - comp)),
				            bin)
				        * Math.pow((Math.pow(sest.get(demographic), A) * Math.pow(test.get(demographic), B)
				                * Math.pow((1 - sest.get(demographic) - test.get(demographic)), C)),
				            1 - bin);
