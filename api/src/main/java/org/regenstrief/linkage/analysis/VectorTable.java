...
/**
 * Class calculates a score, sensitivity, and specificity table for a given set of m and u values
 * from the MatchingConfig object.
 */
...
public class VectorTable {
				...
				if (mr1.getScore() == mr2.getScore()) {
					if (mr1.getTrueProbability() == mr2.getTrueProbability()) {
						if (mr1.getFalseProbability() == mr2.getFalseProbability()) {
							return Double.compare(Double.parseDouble(mr2.getMatchVector().toString()),
							    Double.parseDouble(mr1.getMatchVector().toString()));
						} else {
	...
	private List<MatchResult> getPossibleMatchingResults() {
		...
		for (int i = 0; i < Math.pow(2, demographics.length); i++) {
			...
			mvs.add(new MatchResult(score, incl_score, getMatchVectorTrueProbability(mv), getMatchVectorFalseProbability(mv),
			        0, 0, mv, null, null, null, mc));
		}
