...
/**
 * Class represents the results of matching two Record objects. There is the main score that is a
 * numeric represntation of how well the two records match as well as a Hashtable that stores
 * boolean values for the given demographic indicating whether for the given demographic the two
 * records were a match. TODO: Implement hashCode()
 */
...
public class MatchResult extends RecordLink implements Comparable<MatchResult> {
	 ...
	 */
	public MatchResult(double score, double incl_score, double true_prob, double false_prob, double sensitivity,
	    double specificity, MatchVector match_vct, ScoreVector score_vct, Record r1, Record r2, MatchingConfig mc) {
		super(r1, r2);
		this.score = score;
	...
	/**
	 * Returns a list of the keys in the match table that contain the demographics used in this record
	 * matching
	 * 
	...
	/**
	 * Method returns the value between 0 and 1 that the used comparator calculated for the given
	 * demographic and the two values for the two records
	 * 
	 * @param demographic the demographic of interest
	 * @return the similarity of the two values using the comparator designated in the MatchingConfig
	 *         object
	 */
