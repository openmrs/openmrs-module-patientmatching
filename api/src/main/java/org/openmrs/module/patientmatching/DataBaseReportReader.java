...
/**
 * Class to read the reports saved in database. If the report has too many pairs, this will divide
 * them into pages And will show the defined number of pages per each group
 */
public class DataBaseReportReader {
	
	/**
	 * Number of matching record groups to show in a single page. The page will display roughly twice
	 * the amount of records as this number as page breaks are defined by the groups and not by the
	 * pairs. This class builds a map with details of where the pages should be divided and stores it in
	 * the session. That map will be used in the next reads and this avoid recalculating where the pages
	 * are separated and what should be displayed in each given page.
	 */
	...
	/**
	 * Constructor that is called when the report is first displayed. Calculate and build the mapping
	 * for the next pages
	 * 
	 ...
	 * 
	 * @param reportName name of the report to read
	 * @param paginationMap The mapping of records to the pages. This is calculated in the other
	 *            constructor and reused having stored in the session
	 */
	...
	/**
	 * Get the last page number that the report has. This depends on the number of groups to display and
	 * the groups identified by the report
	 * 
	 * @return
	 */
