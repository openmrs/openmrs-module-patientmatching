...
/**
 * Class to calculate various estimations of an strategy to give a feedback on the goodness of the
 * strategy before it is used in de-duplication
 * 
 * @author pulasthi
 */
public class Estimator {
	...
	public void doEstimationsWithBlockingFields(List<String> entryNames) {
		...
		String select;
		if (patientFieldNames.isEmpty() && personFieldNames.isEmpty() && identifierFieldNames.isEmpty()
		        && identifiers.isEmpty() && attrTypes.isEmpty()) {
			select = buildNameQuery(personNameFieldNames);
		} else {
			select = buildComplexQuery(attributes, identifiers, attrTypes, patientFieldNames, personFieldNames,
			    personNameFieldNames, identifierFieldNames);
		}
		runQuery(select);
	}
	...
	private String buildComplexQuery(Set<String> attributes, List<String> identifiers, List<String> attrTypes,
	        Set<String> patientFieldNames, Set<String> personFieldNames, Set<String> personNameFieldNames,
		...
		if (!attrTypes.isEmpty()) {
			...
			if (!select.contains("PersonAttributeType")) {
				...
				where += " and pat1 = pa1.attributeType and pat2 = pa2.attributeType" + " and pat1.name = pat2.name";
			}
	...
	private String buildNameQuery(Set<String> attributes) {
		StringBuilder select = new StringBuilder(
		        "select count(pn1) from PersonName pn1, PersonName pn2 where pn1.person <> pn2.person");
	...
	/**
	 * Calculates the estimated comparisons that the strategy will run using the configurations user has
	 * specified.
	 * 
	...
	/**
	 * Update the estimation information of given list of PatientMatchingConfiguration This update is
	 * done only if the total patient records is changed by RECALCULATE_IGNORE_FRACTION (currently 0.1)
	 * of the number of records at the last time of the strategy update. Else it is kept unchanged to
