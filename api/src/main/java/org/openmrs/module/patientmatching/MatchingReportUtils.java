 ...
 * runs in the configuration file.
 */
public class MatchingReportUtils {
	...
	/**
	 * Initial status of creating report process. The following are three main status of the process.
	 */
	...
	public static final String MATCHED_RECORDS_STORE = "matchedRecordsStore";
	
	/**
	 * List of steps in the reporting process.
	 */
	public static final String[] REPORT_GEN_STAGES = { "Read configuration file", "Create scratch table",
	        "Create random sample analyzer", "Create analyzer form pairs", "Create pair data source analyzer",
	        "Create expectation maximization (EM) analyzer", "Analyze pairs", "Score pairs", };
	
	/**
	 * Fields that need to be checked to see the current status of the creating report process
	 */
	...
	//New Method1 2
	public static void ReadConfigFile(Map<String, Object> objects, String[] selectedStrategies) {
	...
	//New Method3 End 4
	public static void CreRanSamAnalyzer(Map<String, Object> objects) {
		RecMatchConfig recMatchConfig = (RecMatchConfig) objects.get("recMatchConfig");
		...
		DedupOrderedDataSourceFormPairs formPairsRandom = new DedupOrderedDataSourceFormPairs(databaseReaderRandom,
		        (MatchingConfig) objects.get("matchingConfig"), recMatchConfig.getLinkDataSource1().getTypeTable());
		RandomSampleAnalyzer rsa = new RandomSampleAnalyzer((MatchingConfig) objects.get("matchingConfig"), formPairsRandom);
	...
	//New Method4 5
	public static void CreAnalFormPairs(Map<String, Object> objects) {
		RecMatchConfig recMatchConfig = (RecMatchConfig) objects.get("recMatchConfig");
	...
	//New Method5 6
	public static void CrePairdataSourAnalyzer(Map<String, Object> objects) {
	...
	//New Method6 7
	public static void CreEMAnalyzer(Map<String, Object> objects) {
	...
	//New Method7 8
	public static void AnalyzingData(Map<String, Object> objects) {
	...
	public static void ScoringData(Map<String, Object> objects) throws IOException {
		...
		OrderedDataSourceReader databaseReaderScore = ((ReaderProvider) objects.get("rp")).getReader(
		    ((DataBaseRecordStore) objects.get("recordStore")).getRecordStoreLinkDataSource(), matchingConfig);
		...
		while ((pair = formPairsScoring.getNextRecordPair()) != null) {
			mss.matchAndStore(pair[0], pair[1], matchingConfig);
		}
		
		databaseReaderScore.close();
		
		objects.put(MATCHED_RECORDS_STORE, mss);
	}
	//New Method8 End 9
	...
	public static void CreatingReport(Map<String, Object> objects) throws IOException {
		...
		MatchedRecordsStore store = (MatchedRecordsStore) objects.get(MATCHED_RECORDS_STORE);
	...
	public static void persistReportToDB(String rptName, List<Set<Long>> matchingPairs, Set<String> includeColumns)
		...
		for (String strategyName : selectedStrategyNamesArray) {
			PatientMatchingConfiguration configuration = reportMetadataService
			        .findPatientMatchingConfigurationByName(strategyName);
