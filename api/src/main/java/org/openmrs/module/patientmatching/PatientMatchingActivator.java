...
/**
 * Class implements the startup and initial methods for the module. When loading, module runs the
 * populateMatchingTable() method to synchronize the OpenMRS patient database with the record
 * linkage table. To do this, the module needs to check that ever patient's openmrs_id exists in the
 * table and add it if it is not present. This can take a long time.
 * 
 * @author jegg
 */
...
public class PatientMatchingActivator extends StaticMethodMatcherPointcutAdvisor implements ModuleActivator, Advisor {
	...
	/**
	 * At startup, module parses the configuration file and makes sure the record linkage table is
	 * populated with the existing patients in OpenMRS.
	 */
	@Override
