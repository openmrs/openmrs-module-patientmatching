...
/**
 * Class provides a singleton for database access to the link table.
 * 
 * @author jegg
 */
...
public class LinkDBConnections {
	...
	/**
	 * Method returns an object holding a MatchFinder object and a RecordDBManager object. The first is
	 * used to find matches, the second is used to add, update, or delete the records used for matching.
	 * 
	...
	/**
	 * Method that sets the flag indicating whether a duplicate removal(deduplication) process is
	 * running to false
	 */
	 ...
	 * 
	 * @return rue if the lock was set, false if a deduplication is already running and lock is
	 *         unavailable
	 */
	...
	/**
	 * Method parses a configuration file. Method assumes that the information under the first
	 * datasource tag in the file is the connection information for the record linkage table. The
	 * columns to be used for matching and the string comparators are also listed in this file.
	 * 
	 * @param config the configuration file with connection and linkage information
	 * @return true if there were no errors while parsing the file, false if there was an exception
	 */
	private boolean parseConfig(File config) {
		try {
			...
			finder = new MatchFinder(rmc.getLinkDataSource1(), rp, rmc.getMatchingConfigs(),
			        MatchFinder.Scoring.BLOCKING_EXCLUSIVE);
