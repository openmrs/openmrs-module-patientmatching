...
public class MatchingReportUtilsTest extends BaseModuleContextSensitiveTest {
    ...
    public void persistIncrementalReportToDB_shouldCreateNewReportAndUpdateReport() throws IOException,
        ...
        // Case 4: When a patient which is already in the report as a matched record, is updated in way that the patient
        // no longer exhibits matching properties
        // Field for collection of projections

        // Get the patient whose id is 999
        Patient updatedPatient = null;
        for (MatchingRecord mr4 : report3.getMatchingRecordSet()) {
            if (mr4.getPatient().getPatientId() == 999) {
                updatedPatient = mr4.getPatient();
            }
        }
        ...
        Field fieldListAddedUpdated = MatchingReportUtils.class.getDeclaredField("listAddedUpdatedPatients");
        fieldListAddedUpdated.setAccessible(true);
        List<Patient> updatedList = new ArrayList<Patient>();
        updatedList.add(updatedPatient);
        fieldListAddedUpdated.set(null, updatedList);

        // Call the method
        MatchingReportUtils.persistIncrementalReportToDB(reportName, new ArrayList<Set<Long>>(), includeColumns,
                matchingConfigLists);
        Report report4 = reportMetadataService.getReportByName(reportName);

        for (MatchingRecord mr5 : report4.getMatchingRecordSet()) {
            // Asserts
            int patientId = mr5.getPatient().getPatientId();
            // Patients whose ids are 999 and 7 should not be in the report since they were the matched pair in report3
            Assert.assertNotEquals(999, patientId);
            Assert.assertNotEquals(7, patientId);
        }
