...
public class ScorePair {
	...
	{
		if(!(rec1.getDemographic("concat1").equals(""))&(rec2.getDemographic("concat1").equals("")))
		{
			 String data1 = rec1.getDemographic("concat1");
			 String data2 = rec2.getDemographic("concat1");
			 boolean match = StringMatch.LCSMatch(data1, data2, Double.parseDouble(Context.getAdministrationService().getGlobalProperty("patientmatching.algorithm.lcs.threshold")));
		if(match)
		{
			List<MatchingConfigRow> interchangeableColumns = mc.getInterchangeableColumns();
			for(MatchingConfigRow mcr : interchangeableColumns)
			{
				mv.setMatch(mcr.getName(),match);
			}
		}
		return mv;
		}
