...
/**
 * Class represents a set of Records that are all the same thing and how that
 * decision was made.  One link between Records consist of just two Records, A and B.
 * If A,B is a link, and B,C is a different link, this class holds the information that A,B,C 
 * is all the same entity.  The class's data can be generated automatically, like from a list 
 * of RecordLink objects, or explicitly by calling the addRecordToGroup method.
 *
 */
...
public class SameEntityRecordGroup {
	...
	/**
	 * Returns the integr group ID for the set of equivalent Records.  ID should be
	 * unique among groups created during matching
	 * 
	...
	/**
	 * Method returns the RecordLink objects that link all of the Records in the group together.
	 * Every record returned by getGroupRecords should be present in one side of a RecordLink,
